...
public class EmployeeEditController implements Initializable, MenuForm {
	...
	@FXML
	public ComboBox<PhoneType> phone1TypeComboBox;
	@FXML
	public TextField phone2TextField;
	@FXML
	public ComboBox<PhoneType> phone2TypeComboBox;
	@FXML
	public TextField phone3TextField;
	@FXML
	public ComboBox<PhoneType> phone3TypeComboBox;
	@FXML
	...
	protected void createDOBComboBoxes () {
		this.dobValue = null;
	...
	protected void invalidInputValueOnEmployeeAdd (String message, Control targetController) {
		new Alert(Alert.AlertType.WARNING, message).show();

		targetController.requestFocus();
	}

	private boolean isInvalidPhoneNumber (int index, String phone, PhoneType type, Control targetController, InputValidator inputValidator) {
		if ((type.equals(PhoneType.MOBILE) || type.equals(PhoneType.WHATSAPP))) {
	...
	private boolean validatePhoneNumbers (String phone1, String phone2, String phone3, PhoneType phone1Type, PhoneType phone2Type, PhoneType phone3Type, InputValidator inputValidator, boolean asUpdate) {
		if (
	...
	protected Employee validateInputsAndGetNewEmployee (boolean asUpdate) {
		...
		final PhoneType phone1Type = this.phone1TypeComboBox.getValue();
		final PhoneType phone2Type = this.phone2TypeComboBox.getValue();
		...
		if (!asUpdate && this.employeeService.isUsernameAvailable(userName)) {
		...
		if (!asUpdate && this.employeeService.isNICAvailable(nic)) {
		...
		if (!this.validatePhoneNumbers(phone1, phone2, phone3, phone1Type, phone2Type, phone3Type, inputValidator, asUpdate)) return null;
		...
		if (!phone1.isEmpty()) phones.add(EmployeePhone.builder().phone(phone1).type(phone1Type.toString().toUpperCase()).build());
		if (!phone2.isEmpty()) phones.add(EmployeePhone.builder().phone(phone2).type(phone2Type.toString().toUpperCase()).build());
		if (!phone3.isEmpty()) phones.add(EmployeePhone.builder().phone(phone3).type(phone3Type.toString().toUpperCase()).build());
